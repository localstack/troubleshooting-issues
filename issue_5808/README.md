...
```bash
localstack-bug-node14$ sam build
```
...
```bash
localstack-bug-node14$ sam logs -n StockCheckerFunction --stack-name localstack-bug-node14 --tail
```
...
```bash
localstack-bug-node14$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name localstack-bug-node14
```
